...
export const player_entity = (() => {
  ...
  class BasicCharacterController extends entity.Component {
    ...
    Update(timeInSeconds) {
      ...
      if (collisions.length > 0 || this.checkForWall(pos) === true) {
        return
      }

      if(this.sendLeaderboard === true) {
        if (pos.z > 505) {
          this.sendLeaderboard = false;
          const msg = 'Has finished the Maze in Position'
          if (msg != '') {
            const net = this.FindEntity('network').GetComponent('NetworkController')
            net.UpdateLeaderboard(msg)
          }
        }
      }
    ...
    checkForWall(pos) {
      if (pos.z > 145 && pos.z < 154 && pos.x > -50) {
        //left maze entrance
        return true
      }

      if (pos.z > 145 && pos.z < 154 && pos.x < -98) {
        //right maze entrance
        return true
      }

      if (pos.z > 495 && pos.z < 504 && pos.x > -50) {
        //left Exit
        return true
      }

      if (pos.z > 495 && pos.z < 504 && pos.x < -98) {
        //Right Exit
        return true
      }

      if (pos.x > 145 && pos.z < 145) {
        return true
      }

      if (pos.x < -295 && pos.z < 145) {
        return true
      }

      if (pos.x < 145 && pos.x > -295 && pos.z < -195) {
        return true
      }

      if (pos.x > 295 || pos.x < -295) {
        return true
      }
